...
async function main() {
  const userDataDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "huma-phase2-console-check-"),
  );
  ...
  try {
    ...
    await send("Page.navigate", { url: "http://127.0.0.1:4173/#contact" });
    await waitForLoad();
    const contact = await getPageState();
