// Vercel Node serverless function adapter. Deployed at POST /api/contact
// once Phase 16 (deployment) is approved — inert until then. Real logic
// lives in server/api/contact.mjs.
...
export default async function handler(req, res) {
  ...
  const { status, body } = await handleContactRequest({ rawBody: req.body, serializedLength, clientKey });
